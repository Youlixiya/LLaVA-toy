...
from EdgeSAM.web_demo.utils.tools_gradio import fast_process
import copy
import argparse
# from liga.clip_resnet import TinyCLIPSAMImageEncoder, OVSAMImageEncoder
# from liga.openclip_convnext_large import CLIPConvNeXt
from llava.model import SAMLlavaLlamaForCausalLM
...
parser.add_argument(
    "--checkpoint",
    default="checkpoints/sam/edge_sam_3x.pth",
    type=str,
...
if args.enable_onnx:
    ...
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # image_encoder = TinyCLIPSAMImageEncoder('checkpoints/tinyclip/tinyclip_resnet.pt')
    # image_encoder.load_state_dict(torch.load('checkpoints/distillation_tinyclip_sam_image_encoder.pt'))
    # image_encoder = CLIPConvNeXt()
    # image_encoder.load_state_dict(torch.load('checkpoints/distillation_clipconvnext_edgesam_image_encoder.pt'))
    # image_encoder = OVSAMImageEncoder()
    # sam = sam_model_registry["edge_sam"](checkpoint=args.checkpoint, upsample_mode="bicubic")
    # sam = sam_model_registry["vit_h"](checkpoint='ckpts/sam/sam_vit_b_01ec64.pth')
    # sam.image_encoder = image_encoder
    # sam = sam.to(device=device)
    # sam.eval()
    # model_args = {
    #     'sam_adapter_ckpts': 'ckpts/sam/sam_adapter.pt',
    #     'custom_sam_ckpts': 'ckpts/sam/custom_sam.pt'
    # }
    model = SAMLlavaLlamaForCausalLM.from_pretrained('checkpoints/sam-llava-openclip-convnext-tinyllama-v1.0-1.1b-3t')
    model.load_sam_image_encoder()
    predictor = model.sam_predictor


examples = [
    ["EdgeSAM/web_demo/assets/1.jpeg"],
    ["EdgeSAM/web_demo/assets/2.jpeg"],
    ["EdgeSAM/web_demo/assets/3.jpeg"],
    ["EdgeSAM/web_demo/assets/4.jpeg"],
    ["EdgeSAM/web_demo/assets/5.jpeg"],
    ["EdgeSAM/web_demo/assets/6.jpeg"],
    ["EdgeSAM/web_demo/assets/7.jpeg"],
    ["EdgeSAM/web_demo/assets/8.jpeg"],
    ["EdgeSAM/web_demo/assets/9.jpeg"],
    ["EdgeSAM/web_demo/assets/10.jpeg"],
    ["EdgeSAM/web_demo/assets/11.jpeg"],
    ["EdgeSAM/web_demo/assets/12.jpeg"],
    ["EdgeSAM/web_demo/assets/13.jpeg"],
    ["EdgeSAM/web_demo/assets/14.jpeg"],
    ["EdgeSAM/web_demo/assets/15.jpeg"],
    ["EdgeSAM/web_demo/assets/16.jpeg"]
]
